...
private:
	...
	// Sift item down in heap until everything below it is greater
	...
	// Sift item in heap upward until it's greater than it's parent.
	void siftUp(int pos) {
...
public:
	...
	MinHeap(E* h = nullptr, int num = NULL, int max = NULL) {
		heap = h;
		size = num;
		maxSize = max;
		buildHeap();
	...
	// Pop first (min) item. Swap root and last element, then sift down old last element.
	E removeFirst() {
